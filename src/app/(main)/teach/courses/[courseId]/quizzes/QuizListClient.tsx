...
export function QuizListClient({ course }: { course: Course }) {
  ...
  return (
    <div className="max-w-4xl mx-auto p-8 lg:p-12">
      ...
      {/* Stats */}
      <div className="bg-white rounded-2xl p-6 border border-neutral-100 mb-6 grid grid-cols-3 gap-6">
        <div>
          <p className="text-2xl font-black font-display">{totalQuizzes}</p>
          <p className="text-sm text-neutral-600">Quiz đã tạo</p>
        </div>
        <div>
          <p className="text-2xl font-black font-display">
            {course.sections.reduce((acc, s) => acc + s.lessons.length, 0)}
          </p>
          <p className="text-sm text-neutral-600">Tổng bài học</p>
        </div>
        <div>
          <p className="text-2xl font-black font-display">
          ...
          <p className="text-sm text-neutral-600">Lượt làm bài</p>
        </div>
      </div>
...
function LessonRow({
  ...
  return (
    <div className="p-4 flex items-center gap-3 hover:bg-neutral-50">
