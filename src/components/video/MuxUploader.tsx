...
export function MuxUploader({
  ...
  if (currentStatus === 'ERRORED' && !uploading) {
    return (
      <div className="border-2 border-red-200 bg-red-50 rounded-xl p-4 flex items-center gap-3">
  ...
  if (uploading) {
    return (
      <div className="border-2 border-yellow-400 bg-yellow-50 rounded-xl p-6">
        ...
        <p className="text-xs text-neutral-600 mt-2 text-right">{progress}%</p>
      </div>
    );
  }
  ...
  return (
    <label
      className={`block border-2 border-dashed rounded-xl p-8 cursor-pointer transition-colors border-neutral-300 hover:border-black hover:bg-neutral-50`}
    >
...
function FileInput({ onSelect }: { onSelect: (file: File) => void }) {
  return (
    <label className="px-3 py-1.5 text-xs font-bold border border-neutral-200 rounded-lg cursor-pointer hover:border-black">
      Upload lại
      <input
        type="file"
        accept="video/*"
